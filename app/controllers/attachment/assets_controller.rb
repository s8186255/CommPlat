...
class Attachment::AssetsController < ApplicationController
  ...
  def create
    @attachment = params[:upfile]
    @type = params[:type]
    @type = 'image'
    unless @attachment.nil?
      if Attachment::AssetUploader.save_upload_info? # save upload info into database
        # begin
          @asset = "Attachment::#{@type.camelize}".constantize.new(:asset => @attachment)
          ...
          @asset.save
          logger.warn @asset.asset.url
          render :text => ({url: @asset.asset.url,background_img_id:@asset.id.to_s,size: @asset.file_size,type: ".jpg",state: "SUCCESS",name: "hello.jpg",originalName: "world.jpg"}.to_json)
        #   if @asset.save
        ...
        begin
          uploader = "Attachment::#{@type.camelize}Uploader".constantize.new
